...
assert(region > 0);
if (sfp->isNib)
    seq = nibLdPartMasked(NIB_MASK_MIXED, nibFile, f, seqSize, 
          seqStart, region);
else
    readCachedSeqPart(chrom, seqStart, region, 
          tHash, fileCache, &seq, &tOffset, &tIsNib);
if (strand[0] == '+')
    {
...
//pslMergeBlocks(target, targetM, maxBlockGap);
//pslMergeBlocks(query, queryM, maxBlockGap);
verbose(4, "countRetainSS t strand %s %s %d blocks ",target->strand, target->tName, targetM->blockCount);
printPslQBlocks(target, TRUE);
verbose(4, "countRetainSS q strand %s %s ",query->strand, query->tName);
printPslQBlocks(query, TRUE);
assert(target!=NULL);
    ...
    for (i = 0 ; i < targetM->blockCount ; i++)
      {
        ...
        int teReps = 0;
        int tdiff = ts - prevEnd;
        qs = targetM->qStarts[i];
        verbose(6, "%s %s qs %d i=%d qStarts %d blksz %d\n",targetM->tName, targetM->qName, qs, i, targetM->qStarts[i], targetM->blockSizes[i]);
        /* combine blocks that are close together */
        while (tdiff < maxBlockGap && i< (targetM->blockCount)-1)
            {
            i++;
                verbose(4, " COUNT EXON COMBINE prevEnd %d  ts %d te %d diff %d  %s\n",
                                prevEnd, ts, te, tdiff, query->qName);
            tdiff = targetM->tStarts[i]-te;
            te = targetM->tStarts[i] + targetM->blockSizes[i];
            qe = targetM->qStarts[i] + targetM->blockSizes[i];
            }
        /* skip exons that overlap repeats */
        if (i < (targetM->blockCount) -1)
            {
            teReps += countRepeatOverlap(targetM->tName, ts, te);
            *tReps += teReps;
            if (2 * teReps > (te-ts))
                continue;
            }
        verbose(6, "   before qs %d qe=%d sz %d\n",qs, qe, targetM->qSize);
        if (targetM->strand[0] == '-')
            reverseIntRange(&qs, &qe, targetM->qSize);
        verbose(6, "   after qs %d qe=%d sz %d\n",qs, qe, targetM->qSize);
        ...
        if (positiveRangeIntersection(query->qStart, query->qEnd, qs, qe) > min(20,qe-qs))
            ...
            for (j = 0 ; j < query->blockCount ; j++)
                {
                int localReps = countRepeatOverlap(query->tName, query->tStarts[j], 
                        query->tStarts[j]+query->blockSizes[j]);
                *qReps += localReps;
                qqs = query->qStarts[j];
                ...
                if (positiveRangeIntersection(qqs, qqe, qs, qe) > 10)
                    {
                    count++;
                    verbose(4, " COUNT EXON INTERSECT %d >10  qqs %d qqe %d target: qs %d qe %d ts %d te %d diff %d  %s\n",
                                positiveRangeIntersection(qqs, qqe, qs, qe), qqs,qqe,qs,qe, ts, te, tdiff, query->qName);
                    break;
                    }
                else
                    verbose(6, " count exon not intersect %d <10  qqs %d qqe %d target: qs %d qe %d ts %d te %d diff %d %s\n",
                                positiveRangeIntersection(qqs, qqe, qs, qe), qqs,qqe,qs,qe, ts, te, tdiff, query->qName);
                }
            }
        else
            {
            verbose(6, " count exon not intersect %d < min(20,qe-qs %d) query %d-%d target %d %d ts %d te %d diff %d %s\n",
                  positiveRangeIntersection(query->qStart, query->qEnd, qs, qe) , 
                        qe-qs,query->qStart, query->qEnd, qs, qe, ts, te, tdiff, query->qName);
            }
        start = i+1;
        prevEnd = te;
        }
...
//pslFree(&targetM);
//pslFree(&queryM);
verbose(4, "COUNT EXON COVER = %d %s\n",count, query->qName);
return count;
}
